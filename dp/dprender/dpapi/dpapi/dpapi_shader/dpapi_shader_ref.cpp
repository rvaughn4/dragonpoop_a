...
namespace dp
{
    ...
    //ctor
    dpapi_shader_ref::dpapi_shader_ref( dpapi_shader *p, std::shared_ptr<dpshared_ref_kernel> *k, std::shared_ptr< std::atomic<uint64_t> > *t_sync ) : dpshared_ref( p, k, t_sync )
    {
        this->p = p;
    }

    //dtor
    dpapi_shader_ref::~dpapi_shader_ref( void )
    {
